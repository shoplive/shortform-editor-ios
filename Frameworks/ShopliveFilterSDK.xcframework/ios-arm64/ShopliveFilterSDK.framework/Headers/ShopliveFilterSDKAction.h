...
#if !defined(_ShopliveFilterSDKACTION_H_) && !defined(_ShopliveFilterSDK_ONLY_FILTERS_)
#define _ShopliveFilterSDKACTION_H_
...
namespace ShopliveFilterSDK
{
	...
	public:
		...
		void attachControlPoints(std::vector<ShopliveFilterSDKCurveInterface::CurvePoint>& points)
		{
			ShopliveFilterSDKCurveInterface::genCurve(m_curve, points.data(), points.size());
		}

		void attachControlPoints(ShopliveFilterSDKCurveInterface::CurvePoint* points, size_t size)
		{
			ShopliveFilterSDKCurveInterface::genCurve(m_curve, points, size);
		}

		void attachControlPoints(std::vector<Vec2f>& points)
		{
			int size = (int)points.size();
			std::vector<ShopliveFilterSDKCurveInterface::CurvePoint> controlPoints(size);
			for (int i = 0; i < size; ++i)
			{
				controlPoints[i].x = points[i].x();
				controlPoints[i].y = points[i].y();
			}
			ShopliveFilterSDKCurveInterface::genCurve(m_curve, controlPoints.data(), size);
		}
	...
	public:
		...
		void act(float percent)
		{
			...
			int section1 = t * sum, section2 = ShopliveFilterSDK_MIN(section1, sum);
	...
	public:
		...
		void act(float percent)
		{
			...
			int sum = (int)m_curve.size() - 1;
			int section1 = t * sum, section2 = ShopliveFilterSDK_MIN(section1+1, sum);
			float sectionStart = m_curve[section1];
			float sectionEnd = m_curve[section2];
			float p = t * sum - section1;
	...
	public:
		...
		void act(float percent)
		{
			...
			for (int i = 0; i < m_logicSprites.size(); ++i)
			{
				...
				float minSize = ShopliveFilterSDK::ShopliveFilterSDK_MIN(m_composeSize.x(), m_composeSize.y());
				float maxSize = ShopliveFilterSDK::ShopliveFilterSDK_MAX(m_composeSize.x(), m_composeSize.y());
				...
				topPos = Mat4::makeScale(maxSize/2.0f, maxSize/2.0f, 1.0) * topPos;
				{
					...
					float factor = ShopliveFilterSDK::ShopliveFilterSDK_MIN(m_viewportSize.x(), m_viewportSize.y()) / minSize;
					centerPos.x() *= factor;
					centerPos.y() *= factor;
					rightPos.x() *= factor;
					topPos.y() *= factor;
