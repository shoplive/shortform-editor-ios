/*
* ShopliveFilterSDKShaderFunctions.h
*
*  Created on: 2013-12-6
*      Author: Wang Yang
*/

#ifndef _ShopliveFilterSDKSAHDERFUNCTIONS_H_
#define _ShopliveFilterSDKSAHDERFUNCTIONS_H_
	...
	public:
		...
		typedef struct UniformData
		{
			...
			UniformData(const char* name, UNIFORM_TYPE type) :
				uniformType(type){ strncpy(uniformName, name, ShopliveFilterSDK_UNIFORM_MAX_LEN); }
			...
			char uniformName[ShopliveFilterSDK_UNIFORM_MAX_LEN];
		...
		void clear();
		void assignUniforms(ShopliveFilterSDKImageHandlerInterface* handler, GLuint programID);
	...
	protected:
		inline GLint _getUniform(GLuint programId, const char* name)
		{
			...
			ShopliveFilterSDK_LOG_CODE(
				if(uniform < 0)
					ShopliveFilterSDK_LOG_ERROR("uniform name %s does not exist!\n", name);
			);
			return uniform;
		}
