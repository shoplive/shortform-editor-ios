/*
* ShopliveFilterSDKLiquidation.h
*
...
#ifndef _ShopliveFilterSDKLIQUIDATION_H_
#define _ShopliveFilterSDKLIQUIDATION_H_

#include "ShopliveFilterSDKAdvancedEffectsCommon.h"
#include "ShopliveFilterSDKVec.h"

#define ShopliveFilterSDK_DEFORM_SHOW_MESH

namespace ShopliveFilterSDK
{

	class ShopliveFilterSDKLiquidationFilter : public ShopliveFilterSDKImageFilterInterface
	{
	public:
		ShopliveFilterSDKLiquidationFilter();
		~ShopliveFilterSDKLiquidationFilter();
	...
	protected:
		static ShopliveFilterSDKConstString paramTextureVertexName;
	...
	protected:
		...
		ShopliveFilterSDKSizei m_meshSize;
		std::vector<Vec2f> m_mesh;
		std::vector<std::vector<Vec2f> > m_vecMeshes;
		int m_meshIndexSize;
		bool m_doingRestore;
#ifdef ShopliveFilterSDK_DEFORM_SHOW_MESH
		ProgramObject m_programMesh;
		bool m_bShowMesh;
#endif
	};

	class ShopliveFilterSDKLiquidationNicerFilter : public ShopliveFilterSDKLiquidationFilter
	{
	public:
		void forwardDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius, float intensity);//see in 'ShopliveFilterSDKLiquidationFilter'
		virtual void pushLeftDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius, float intensity, float angle = 3.14159f / 2.0f);//see in 'ShopliveFilterSDKLiquidationFilter'
	};
