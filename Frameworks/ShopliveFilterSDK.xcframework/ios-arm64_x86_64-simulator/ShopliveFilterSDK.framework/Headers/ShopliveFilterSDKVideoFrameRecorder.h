/*
 * ShopliveFilterSDKVideoFrameRecorder.h
 *
...
#import "ShopliveFilterSDKFrameRecorder.h"
#import "ShopliveFilterSDKCommonDefine.h"

// 'ShopliveFilterSDKFrameRecorder' is used for offscreen processing, no sounds play back. (Sounds would be saved in the result)

@class ShopliveFilterSDKVideoFrameRecorder;

@protocol ShopliveFilterSDKVideoFrameRecorderDelegate <NSObject>

@required
- (void)videoReadingComplete:(ShopliveFilterSDKVideoFrameRecorder*)videoFrameRecorder;
...
@interface ShopliveFilterSDKVideoFrameRecorder : ShopliveFilterSDKFrameRecorder
...
@property(nonatomic, weak) id<ShopliveFilterSDKVideoFrameRecorderDelegate> videoFrameRecorderDelegate;
...
- (void)setTimeline:(void*)timeline; //特殊接口， timeline类型必须为 ShopliveFilterSDK::TimeLine.
 ...
 @"videoOrientation" : @(0),                            //视频朝向, 取值0,1,2,3 分别表示旋转0,90,180,270度
  ...
  @"filterIntensity" : (float)filterIntensity,          //滤镜强度 (不写默认 1.0, 范围[0, 1])
       @"blendImage" : (UIImage*)blendImage,            //每一帧混合图片 (可不写)
        @"blendMode" : (ShopliveFilterSDKTextureBlendMode)blendMode,  //混合模式 (默认 ShopliveFilterSDK_BLEND_MIX 当blendImage不存在时无效)
...
+ (instancetype)generateVideoWithFilter:(NSURL *)ouputVideoUrl size:(CGSize)videoSize withDelegate:(id<ShopliveFilterSDKVideoFrameRecorderDelegate>)delegate videoConfig:(NSDictionary *)videoConfig;

+ (instancetype)generateVideoWithFilter:(NSURL *)ouputVideoUrl size:(CGSize)videoSize withDelegate:(id<ShopliveFilterSDKVideoFrameRecorderDelegate>)delegate videoConfig:(NSDictionary *)videoConfig outputSetting:(NSDictionary*)outputSetting;
