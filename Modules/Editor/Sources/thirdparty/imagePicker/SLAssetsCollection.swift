...
public struct SLPHAsset {
    ...
    //Bool to check if SLPHAsset returned is created using camera.
    ...
    public var fullResolutionImage: UIImage? {
        get {
            ...
            return SLPhotoLibrary.fullResolutionImageData(asset: phAsset)
        }
    }
    ...
    public func cloudImageDownload(progressBlock: @escaping (Double) -> Void, completionBlock:@escaping (UIImage?)-> Void ) -> PHImageRequestID? {
        guard let phAsset = self.phAsset else { return nil }
        return SLPhotoLibrary.cloudImageDownload(asset: phAsset, progressBlock: progressBlock, completionBlock: completionBlock)
    }
    ...
    public static func asset(with localIdentifier: String) -> SLPHAsset? {
        let fetchResult = PHAsset.fetchAssets(withLocalIdentifiers: [localIdentifier], options: nil)
        return SLPHAsset(asset: fetchResult.firstObject)
    }
}

extension SLPHAsset: Equatable {
    public static func ==(lhs: SLPHAsset, rhs: SLPHAsset) -> Bool {
...
public struct SLAssetsCollection {
    ...
    public var sections: [(title: String, assets: [SLPHAsset])]? = nil
    var count: Int {
        get {
    ...
    func getSLAsset(at indexPath: IndexPath) -> SLPHAsset? {
        ...
        if let sections = self.sections {
            ...
            guard let result = self.fetchResult, index < result.count else { return nil }
            return SLPHAsset(asset: result.object(at: max(index,0)))
        }
    }
    ...
    mutating func reloadSection(groupedBy: PHFetchedResultGroupedBy) {
        ...
        if self.useCameraButton {
            groupedSections.insert(("camera",[SLPHAsset(asset: nil)]), at: 0)
        }
        self.sections = groupedSections
    }
    
    static func ==(lhs: SLAssetsCollection, rhs: SLAssetsCollection) -> Bool {
