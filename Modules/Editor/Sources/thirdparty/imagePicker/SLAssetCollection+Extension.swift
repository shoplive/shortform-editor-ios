//
//  SLAssetCollection+Extension.swift
//  SLPhotoPicker
//
...
extension SLAssetsCollection {
    func enumarateFetchResult(groupedBy: PHFetchedResultGroupedBy) -> Dictionary<String,[SLPHAsset]> {
        ...
        let sections = Dictionary(grouping: assets.map{ SLPHAsset(asset: $0) }) { (element) -> String in
    ...
    func section(groupedBy: PHFetchedResultGroupedBy) -> [(String,[SLPHAsset])] {
        let dict = enumarateFetchResult(groupedBy: groupedBy)
        var sections = [(String,[SLPHAsset])]()
