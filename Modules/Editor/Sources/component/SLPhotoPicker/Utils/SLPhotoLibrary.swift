...
extension SLPhotoLibrary {
    func getOption(configure: SLPhotosPickerConfigure) -> PHFetchOptions {
    ...
    func fetchResult(collection: SLAssetsCollection?, configure: SLPhotosPickerConfigure) -> PHFetchResult<PHAsset>? {
    ...
    func fetchCollection(configure: SLPhotosPickerConfigure) {
        self.albums = nil
        ...
        func getAlbum(subType: PHAssetCollectionSubtype, result: inout [SLAssetsCollection]) {
            ...
            for collection in collections {
                if !result.contains(where: { $0.localIdentifier == collection.localIdentifier }) {
                    var assetsCollection = SLAssetsCollection(collection: collection)
        ...
        func getSmartAlbum(subType: PHAssetCollectionSubtype,
                           ...
                           result: inout [SLAssetsCollection])
            -> SLAssetsCollection?
        {
        ...
        DispatchQueue.global(qos: .userInteractive).async { [weak self] in
            var assetCollections = [SLAssetsCollection]()
