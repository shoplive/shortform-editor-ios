...
class SLVideoEditorSliderView2 : UIView, SLReactor {
    ...
    private lazy var thumbnailHandleView : SLThumbnailSliderHandleView = {
    ...
    private var isTimeIndicatorDragging : Bool = false
    ...
    init(videoUrl: URL) {
        ...
        handleView.setPixelPerTime(pixelPerTime: self.pixelPerTime)
...
//        handleView.updateTimeIndicatorTime(time: time)
    ...
    private func onInitializeThumbnailHandleView() {
        handleView.initializeThumbView()
        //MARK: - TODO 개발용 코드
        thumbnailHandleView.isHidden = true
...
extension SLVideoEditorSliderView2 {
    ...
    private func calculateFrameForMaxTrimTimeBiggerThenVideoDuration() {
        ...
        self.pixelPerTime = ( fullTrimWidth / self.videoDuration ) // 1초당 pixel
...
extension SLVideoEditorSliderView2  {
    ...
    private func bindHandleView() {
        handleView.resultHandler = { [weak self] result in
            ...
            case .handleDidFinishDragging:
                self.onHandleHandleDidFinishDragging()
    ...
    private func onHandleHandleDidStartDragging() {
        resultHandler?( .toggleViewPlayOrPause )
    }
    
    private func onHandleHandleDidFinishDragging() {
        resultHandler?( .toggleViewPlayOrPause )
    }
    
    private func onHandleDidFinishDragging() {
        resultHandler?( .toggleViewPlayOrPause )
        self.isTimeIndicatorDragging = false
    }
    
    private func onHandleDidStartDragging() {
        resultHandler?( .toggleViewPlayOrPause)
        self.isTimeIndicatorDragging = true
    }
