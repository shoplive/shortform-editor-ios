//
//  SLVideoFilterViewController.swift
...
class SLVideoFilterViewController : UIViewController {
   ...
   lazy private var playerView : ShopLiveFilterPlayer = {
    ...
    private let reactor : SLVideoFilterReactor
    
    var delegate : SLVideoFilterViewControllerDelegate?
    
    
    required init(videoEditInfo : SLVideoEditInfoDTO, thumbnailImage : UIImage) {
        self.reactor = SLVideoFilterReactor(videoEditInfoDto: videoEditInfo, thumbNailImage: thumbnailImage)
    ...
    private func onReactorRequestOnConfirm(didChange : Bool) {
        self.delegate?.filterViewController(didFinish: didChange)
    }
...
extension SLVideoFilterViewController {
    ...
    private func onSliderCurrentValue(value : CGFloat) {
        ...
        playerView.action( .setFilterIntensity(Float(value)) )
        sliderView.action( .setValueLabel(String(format: "%.1f", value)) )
    }
}
extension SLVideoFilterViewController {
...
extension SLVideoFilterViewController {
    private func setLayout() {
        ...
        NSLayoutConstraint.activate([
            ...
            filterCv.leadingAnchor.constraint(equalTo: self.view.leadingAnchor),
            filterCv.trailingAnchor.constraint(equalTo: self.view.trailingAnchor),
            filterCv.bottomAnchor.constraint(equalTo: bottomBar.topAnchor,constant: -20),
            filterCv.heightAnchor.constraint(equalToConstant: 96),
            ...
            sliderView.bottomAnchor.constraint(equalTo: filterCv.topAnchor,constant:  -24),
