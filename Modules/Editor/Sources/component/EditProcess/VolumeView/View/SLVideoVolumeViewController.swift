//
//  SLVideoVolumeViewController.swift
...
class SLVideoVolumeViewController : UIViewController {
    ...
    private let reactor : SLVideoVolumeReactor
    
    var delegate : SLVideoVolumeViewControllerDelegate?
    
    
    required init(videoEditInfoDto : SLVideoEditInfoDTO){
        self.reactor = SLVideoVolumeReactor(videoEditInfoDto: videoEditInfoDto)
    ...
    private func bindReactor() {
        reactor.resultHandler = { [weak self] result in
            ...
            case .setInitialVolume(let value):
                self.onReactorSetInitialVolume(value: value)
            }
        }
    }
    ...
    private func onReactorRequestOnConfirm(didChange : Bool) {
        self.delegate?.videoVolumeViewController(didFinish: didChange)
//        self.delegate?.videoCropViewController(didFinish: didCrop)
    }

    private func onReactorSetInitialVolume(value : CGFloat) {
        sliderView.action( .setCurrentValue(value) )
        sliderView.action( .setValueLabel(String(Int(floor(value)))) )
    }
}

extension SLVideoVolumeViewController {
...
extension SLVideoVolumeViewController  {
    ...
    private func onSliderCurrentValue(value : CGFloat) {
        ...
        reactor.action( .setVolume(result) )
