...
#ifndef _ShopliveFilterSDK_THREAD_H_
#define _ShopliveFilterSDK_THREAD_H_
...
{
    template<class Type>
    class ShopliveFilterSDKSyncReadWriteQueue
    {
    public:
	...
	/*
	class:     ShopliveFilterSDKThreadPreemptive

	适用场景： 屏幕实时刷新、slider实时更新结果等
	
	任务简介： 任务具有持续性， 长时间执行类似任务
	           并且当新任务出现时， 可以马上 丢弃正在执行的任务 或 顶替排队中的任务， 等待中的任务永远不会超过一个。
			   一个 ShopliveFilterSDKThreadPreemptive 实体最多创建一个线程。

	注意： 最好不要在多个线程同时调用 同一个 ShopliveFilterSDKThreadPreemptive 的 API
	*/

	class ShopliveFilterSDKThreadPreemptive
	{
	public:
		ShopliveFilterSDKThreadPreemptive();
		~ShopliveFilterSDKThreadPreemptive();
	...
	class ShopliveFilterSDKThreadPool
	{
	private:
		class Worker;
	...
	public:
		...
		ShopliveFilterSDKThreadPool(std::list<std::unique_ptr<Worker>>::size_type maxThreadNum = 1);
		~ShopliveFilterSDKThreadPool();
        ...
        static ShopliveFilterSDKThreadPool* globalThreadPool;
	...
	class ShopliveFilterSDKThreadPool::Work
	{
	public:
	...
	class ShopliveFilterSDKThreadPool::Worker
	{
	public:
		Worker(ShopliveFilterSDKThreadPool& pool);
	...
	private:
		std::thread* m_thread;
		ShopliveFilterSDKThreadPool& m_pool;
		bool m_runningStatus, m_shouldLeave;
	};
}

#define ShopliveFilterSDK_THREAD_WORK ShopliveFilterSDK::ShopliveFilterSDKThreadPool::Work
