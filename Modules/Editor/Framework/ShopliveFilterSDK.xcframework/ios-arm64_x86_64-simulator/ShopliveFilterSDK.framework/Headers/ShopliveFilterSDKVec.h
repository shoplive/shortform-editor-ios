...
#ifndef _ShopliveFilterSDK_VEC_H_
#define _ShopliveFilterSDK_VEC_H_

#include <cmath>
#include "ShopliveFilterSDKStaticAssert.h"

#ifndef ShopliveFilterSDK_CLAMP

#define ShopliveFilterSDK_CLAMP(n, low, high) do{\
if(n < low) n = low;\
...
namespace ShopliveFilterSDK
{
	...
	struct VecAlgorithmHelp<V, 1>
	{
		...
		{
			ShopliveFilterSDK_CLAMP(v[0], low, high);
		}
	};
	...
	struct VecAlgorithmHelp<V, 2>
	{
		...
		{
			ShopliveFilterSDK_CLAMP(v[0], low, high);
			ShopliveFilterSDK_CLAMP(v[1], low, high);
		}
	};
	...
	struct VecAlgorithmHelp<V, 3>
	{
		...
		static inline void clamp(V& v, typename V::VecDataType low, typename V::VecDataType high)
		{
			ShopliveFilterSDK_CLAMP(v[0], low, high);
			ShopliveFilterSDK_CLAMP(v[1], low, high);
			ShopliveFilterSDK_CLAMP(v[2], low, high);
		}
	};
	...
	struct VecAlgorithmHelp<V, 4>
	{
		...
		static inline void clamp(V& v, typename V::VecDataType low, typename V::VecDataType high)
		{
			...
			ShopliveFilterSDK_CLAMP(v[1], low, high);
			ShopliveFilterSDK_CLAMP(v[2], low, high);
			ShopliveFilterSDK_CLAMP(v[3], low, high);
		}
...
namespace ShopliveFilterSDK
{
	...
	public:
		...
		Vec() { ShopliveFilterSDKStaticAssert(DIM > 0 && DIM <= 4);}
		Vec(const Vec& v) { VecAlgorithmHelp<Vec<Type, DIM>, DIM>::_assign(*this, v); }
		Vec(Type x)
		{
			ShopliveFilterSDKStaticAssert(DIM == 1); //DIM should be 1
			m_data[0] = x;
		}

		Vec(Type x, Type y)
		{
			ShopliveFilterSDKStaticAssert(DIM == 2); //DIM should be 2
			m_data[0] = x;
			m_data[1] = y;
		}
		...
		{
			ShopliveFilterSDKStaticAssert(DIM == 3); //DIM should be 3
			m_data[0] = x;
			m_data[1] = y;
			m_data[2] = z;
		}
		...
		{
			ShopliveFilterSDKStaticAssert(DIM == 4); //DIM should be 4
			m_data[0] = x;
			m_data[1] = y;
			m_data[2] = z;
			m_data[3] = w;
		...
		inline Type& x() { ShopliveFilterSDKStaticAssert(DIM >= 1); return m_data[0]; }
		inline Type& y() { ShopliveFilterSDKStaticAssert(DIM >= 2); return m_data[1]; }
		inline Type& z() { ShopliveFilterSDKStaticAssert(DIM >= 3); return m_data[2]; }
		inline Type& w() { ShopliveFilterSDKStaticAssert(DIM >= 4); return m_data[3]; }
		inline Type& r() { ShopliveFilterSDKStaticAssert(DIM >= 1); return m_data[0]; }
		inline Type& g() { ShopliveFilterSDKStaticAssert(DIM >= 2); return m_data[1]; }
		inline Type& b() { ShopliveFilterSDKStaticAssert(DIM >= 3); return m_data[2]; }
		inline Type& a() { ShopliveFilterSDKStaticAssert(DIM >= 4); return m_data[3]; }

		inline const Type& x() const { ShopliveFilterSDKStaticAssert(DIM >= 1); return m_data[0]; }
		inline const Type& y() const { ShopliveFilterSDKStaticAssert(DIM >= 2); return m_data[1]; }
		inline const Type& z() const { ShopliveFilterSDKStaticAssert(DIM >= 3); return m_data[2]; }
		inline const Type& w() const { ShopliveFilterSDKStaticAssert(DIM >= 4); return m_data[3]; }
		inline const Type& r() const { ShopliveFilterSDKStaticAssert(DIM >= 1); return m_data[0]; }
		inline const Type& g() const { ShopliveFilterSDKStaticAssert(DIM >= 2); return m_data[1]; }
		inline const Type& b() const { ShopliveFilterSDKStaticAssert(DIM >= 3); return m_data[2]; }
		inline const Type& a() const { ShopliveFilterSDKStaticAssert(DIM >= 4); return m_data[3]; }
        ...
        inline operator CGPoint() const { ShopliveFilterSDKStaticAssert(DIM == 2); return CGPointMake(m_data[0], m_data[1]); }
        inline operator CGSize() const { ShopliveFilterSDKStaticAssert(DIM == 2); return CGSizeMake(m_data[0], m_data[1]); }
        inline Vec& operator=(const CGPoint& p) { ShopliveFilterSDKStaticAssert(DIM == 2); m_data[0] = p.x; m_data[1] = p.y; return *this; }
        inline Vec& operator=(const CGSize& p) { ShopliveFilterSDKStaticAssert(DIM == 2); m_data[0] = p.width; m_data[1] = p.height; return *this; }
