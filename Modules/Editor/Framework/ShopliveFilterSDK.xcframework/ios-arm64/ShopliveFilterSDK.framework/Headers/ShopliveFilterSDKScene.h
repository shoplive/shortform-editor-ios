...
#ifndef _ShopliveFilterSDK_SCENE_H_
#define _ShopliveFilterSDK_SCENE_H_
...
namespace ShopliveFilterSDK
{
	...
	public:
		...
		{
			m_fovyRad = ShopliveFilterSDK_MID<float>(m_fovyRad + rad, M_PI / 100.0f, M_PI / 2.0f);
			updatePerspective();
		}

		inline void lookInTo(float rad)
		{
			m_fovyRad = ShopliveFilterSDK_MID<float>(rad, M_PI / 100.0f, M_PI / 2.0f);
			updatePerspective();
		}
	...
	public:
		...
		{
			ShopliveFilterSDKAssert(v[0] || v[1]);
		...
		inline void setLookdirZ(float z)
		{
			...
			m_lookZ = ShopliveFilterSDK_MID<float>(z, -lookUpMax, lookUpMax);
		}
		...
		inline void setLookdir(const Vec3f& v)
		{
			...
			m_lookDir *= m_dirLen;
			const float lookUpMax = m_dirLen * 3.732f; // tan(PI / 75);
			m_lookZ = ShopliveFilterSDK_MID<float>(v[2], -lookUpMax, lookUpMax);
		}
		...
		{
			const float lookUpMax = m_dirLen * 3.732f; // tan(PI / 75);
			m_lookZ = ShopliveFilterSDK_MID(m_lookZ + motion * m_dirLen, -lookUpMax, lookUpMax);
		}
		...
		{
			m_lookZ = tanf(ShopliveFilterSDK_MID<float>(rad, -M_PI / 2.4f, M_PI / 2.4f)) * m_dirLen;
		}		
	...
	public:
		...
		inline void setLookdir(const Vec3f& v)
		{
			ShopliveFilterSDKAssert(v[0] || v[2]);
			m_lookDir[0] = v[0];
			m_lookDir[1] = v[2];
			m_lookDir.normalize();
			m_lookDir *= m_dirLen;
			const float lookUpMax = m_dirLen * 3.732f; // tan(PI / 75);
			m_lookZ = ShopliveFilterSDK_MID<float>(v[1], -lookUpMax, lookUpMax);
		}
