...
final class ShopLiveChattingWriteView: UIView {
    ...
    class ViewModel {
        ...
        var sendButtonNormalTitle: NSAttributedString {
            ...
            let font = UIFont.systemFont(ofSize: 14, weight: .medium).findAvailableFont_SL()
            
            paragraphStyle.lineHeightMultiple = font.lineHeightMultiple_SL()
            return NSAttributedString(string: "게시", attributes: [NSAttributedString.Key.kern: -0.28, NSAttributedString.Key.paragraphStyle: paragraphStyle, .foregroundColor: UIColor(red: 0, green: 0.471, blue: 1, alpha: 1), .font: font])
        }
        ...
        var sendButtonDisableTitle: NSAttributedString {
            ...
            paragraphStyle.lineHeightMultiple = font.lineHeightMultiple_SL()
            return NSAttributedString(string: "게시", attributes: [NSAttributedString.Key.kern: -0.28, NSAttributedString.Key.paragraphStyle: paragraphStyle, .foregroundColor: UIColor(red: 0.886, green: 0.886, blue: 0.886, alpha: 1), .font: font])
        }
    }
    ...
    let throttle: ShopLiveShortform.Throttle = ShopLiveShortform.Throttle(queue: DispatchQueue.main, delay: 0.3)
    ...
    func orientationChattingWritrViewConstraint() {
        if UIScreen.currentOrientation_SL.deviceOrientation_SL == .landscapeRight {
            chatViewLeading.constant = UIScreen.safeArea_SL.right == .zero ? 0 : UIScreen.safeArea_SL.right / 3
            sendButtonTrailing.constant = -UIScreen.safeArea_SL.right
        } else if UIScreen.currentOrientation_SL.deviceOrientation_SL == .landscapeLeft {
            chatViewLeading.constant = UIScreen.safeArea_SL.left
            sendButtonTrailing.constant = UIScreen.safeArea_SL.left == .zero ? 0 : -(UIScreen.safeArea_SL.right / 3)
        } else {
