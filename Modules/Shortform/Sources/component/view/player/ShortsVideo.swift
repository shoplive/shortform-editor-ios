...
extension ShopLiveShortform {
    public class ShortsVideo {
        ...
        public func seekTo(time: CMTime) {
            ...
            self.player?.seek(to: time, toleranceBefore: .init(seconds: 1, preferredTimescale: 44100),
                              toleranceAfter: .init(seconds: 1, preferredTimescale: 44100)) { [weak self] isFinished in
                guard let self = self else { return }
                if isFinished {
                    if self.seekNotificationEnabled {
                        self.workItem = DispatchWorkItem {
                            NotificationCenter.default.post(Notification(name: Notification.Name("seekFinished"), object: nil))
                        }
                        DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(800), execute: self.workItem!)
                    }
                }
            }
        }
