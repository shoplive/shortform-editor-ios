...
void main() {
    ...
    // gl_FragColor = fragmentColor * blurIntensity;
    // gl_FragColor.rgb = min(gl_FragColor.rgb, color);

    vec4 resultColor = fragmentColor * blurIntensity;
    resultColor.rgb = min(resultColor.rgb,color);
    resultColor.a = 1.0;
