//
//  ShopLiveCheckBoxContainer.swift
//  PlayerDemo2
//
...
final class ShopLiveCheckBoxCustomButton: UIButton {
...
final class ShopLiveCheckBoxContainer: UIView {
    ...
    private var buttonMap: [ShopLiveButtonType: ShopLiveCheckBoxCustomButton] = [:] // 버튼 참조 저장
    ...
    func configureContainer(receive: PublishSubject<ShopLiveButtonReceiveModel>?) {
        buttonReceiveObservable = receive
        setLayout()
        bind()
    }
    
    private func bind() {
        buttonReceiveObservable?.withUnretained(self)
            .subscribe(onNext: { owner, data in
                owner.updateButtonSelection(id: data.type, isSelected: data.isSelected)
            })
    ...
    private func setLayout() {
        ...
        buttons.forEach { value in
            let button = ShopLiveCheckBoxCustomButton(id: value)
            button.setImage(PlayerDemo2Asset.checkNotSelected.image, for: .normal)
            button.setImage(PlayerDemo2Asset.checkSelected.image, for: .selected)
    ...
    private func updateButtonSelection(selectedButton: ShopLiveCheckBoxCustomButton) {
