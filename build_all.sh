...
update_version_files() {
    ...
    if [ -f "$SWIFT_FILE" ]; then
        ...
        # 220-234 라인의 모든 버전 값 업데이트
        if [[ "$OSTYPE" == "darwin"* ]]; then
            # macOS
            sed -i '' '220,234s/return "[^"]*"/return "'"$VERSION"'"/g' "$SWIFT_FILE"
        else
            # Linux
            sed -i '220,234s/return "[^"]*"/return "'"$VERSION"'"/g' "$SWIFT_FILE"
        fi
...
# 버전 형식 검증 (간단한 검증: 숫자.숫자.숫자 형식)
if ! [[ "$DEPLOY_VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+(\.[0-9]+)?$ ]]; then
  echo "❌ 잘못된 버전 형식입니다. (예: 1.8.0)"
  exit 1
fi
